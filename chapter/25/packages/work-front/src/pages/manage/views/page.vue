...
const pageBaseData = reactive<{ name: string; currentVersion: string }>({
  name: '',
  currentVersion: ''
});
...
onMounted(async () => {
  if (route.query.uuid) {
    fetch(`/api/get/page-info/data?uuid=${route.query.uuid}`)
      .then((res) => res.json())
      .then((result) => {
        Message.open({
...
const onSubmitPage = () => {
  ...
  fetch(
    isEditStatus ? '/api/post/page-info/update' : '/api/post/page-info/create',
    {
